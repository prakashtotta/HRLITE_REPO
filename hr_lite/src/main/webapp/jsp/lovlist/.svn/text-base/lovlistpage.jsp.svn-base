...
String sort = "lovListId"; // default don't sort
...
//List dataList = BOFactory.getRefferalRuleBO().getAllRefferalRuleDetailsForPagination(results,startIndex,dir_str,sort_str);
List dataList =BOFactory.getLovBO().getLovDetailsForPagination(user1,results,startIndex,dir_str,sort_str);

int totalSize = 0;

//List allDataList = BOFactory.getRefferalRuleBO().getAllRefferalRuleDetails();
List allDataList = BOFactory.getLovBO().getAllLovDetails(user1);
totalSize = allDataList.size();
	

System.out.println(totalSize);
String[] fields = {"lovListId","lovListCode","lovListValueCode","lovListValueName","status","localeId"};
