...
<% if(applicantList!=null && applicantList.size()>0){%>
	<tr ><td width="20"></td><td width="252" bgcolor="brown"><%=Constant.getResourceStringValue("aquisition.applicant.APPLICANT_NAME",user1.getLocale())%></td><td width="252" bgcolor="brown"><%=Constant.getResourceStringValue("aquisition.applicant.EMAIL_ID",user1.getLocale())%></td></tr>
<%Iterator itr = applicantList.iterator();
				...
				while(itr.hasNext()){
					...
					<tr ><td width="20"><td bgcolor="#B2D2FF"><a href="applicant.do?method=applicantDetails&applicantId=<%=job.getApplicantId()%>&&secureid=<%=job.getUuid()%>" target ="new"><%=job.getFullName()%></a></td><td bgcolor="#B2D2FF"><%=job.getEmail()%></td></tr>
...
<td valign="top">
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="sentemail" disabled="true" value="<%=Constant.getResourceStringValue("user.massemail..send_Batch_Message",user1.getLocale())%>" border="0" alt="<%=Constant.getResourceStringValue("user.massemail..send_Batch_Message",user1.getLocale())%>" title="<%=Constant.getResourceStringValue("user.massemail..send_Batch_Message",user1.getLocale())%>" height="20"  width="19"  class="initial2"  onMouseover="change(event, 'darksalmon')" onMouseout="change(event, '#c00')" />Your message has been sent.
