...
function taskdetails(idvalue){
	...
	GB_showCenter('<%=Constant.getResourceStringValue("task.graybox.jobdetails",user1.getLocale())%>',url,500,750, messageret);
...
document.bulkUploadTaskForm.action = "bulkuploadtask.do?method=mybulktasklistsearch";
...
<br>
<b><%=Constant.getResourceStringValue("job.myexportuploadtask.pagename",user1.getLocale())%> </b> <br>

<body class="yui-skin-sam" onkeypress="onkeypress()">

<html:form action="/bulkuploadtask.do?method=mybulktasklistsearch">

<table border="0" width="100%" class="div">
		
	<tr>
			<td><%=Constant.getResourceStringValue("task.assigned_date",user1.getLocale())%></td>
			<td>
			<select name="cri">
...
<DIV ID="testdiv1" STYLE="position:absolute;visibility:none;background-color:white;layer-background-color:white;"></DIV>
		...
		<tr>
			<td><%=Constant.getResourceStringValue("job.typeofactivity",user1.getLocale())%></td>
			<td>
...
<input type="button" name="resetdata" value="<%=Constant.getResourceStringValue("hr.button.reset",user1.getLocale())%>" onClick="resetdata1()" class="button"/>
</td>
<td>
...
var editUrl = function(elCell, oRecord, oColumn, sData) {
	 elCell.innerHTML = "<a href='#' onClick=taskdetails(" +"'"+ oRecord.getData("bulkuploadtaskid")+ "'" + ")"+ ">" + oRecord.getData("tasktype") + "</a>";
    ...
    // Column definitions
    var myColumnDefs = [
		{key:"bulkuploadtaskid", hidden:true},
             {key:"tasktype", label:"<%=Constant.getResourceStringValue("task.tasktype",user1.getLocale())%>",sortable:true, resizeable:true,formatter:editUrl,width:250},
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/task/mybulkuploadaskslistpage.jsp?searchpagedisplay=<%=searchpagedisplay%>&cri=<%=aform.getAssignedcri()%>&assigneddate=<%=assigneddate%>&tasktype=<%=aform.getTasktype()%>&status=<%=aform.getStatus()%>&ddd="+(new Date).getTime()+"&");
