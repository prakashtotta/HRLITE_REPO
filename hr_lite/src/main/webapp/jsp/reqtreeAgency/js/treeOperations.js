 ...
 */
var simpleTree;
var structureManagerURL = "jsp/reqtreeAgency/manageTreeData.jsp";
...
function TreeOperations()
{
	...
	this.inputText = "<input type='text' id='inputText' size='50' maxlength='200'>";
...
//*******************************************************	
	this.ajaxReq = function(params, url, callback, overrideSuccessFunc)
	{
		if (treeOps.ajaxActive == true)
		{
			...
			if(params.indexOf('changeOrder')>0){
				var ok = confirm("This operation is not allowed.");
					window.location.reload();
				//window.location.reload();
				//,params.indexOf("&destOwnerEl=")
				var paramtmp = params.substr(params.indexOf("elementId=")+10);
				var elementIdtmp = paramtmp.substr(0,paramtmp.indexOf("&destOwnerEl="));
				var desttoot1 = params.substr(params.indexOf("&destOwnerEl=")+12);
				var desttoot2 = desttoot1.substr(0,desttoot1.indexOf("&position"));
				
				
				if(elementIdtmp.indexOf("_")>0){
					var ok = confirm("Only you can move applicants, not folders");
					window.location.reload();
				}else if(desttoot2 == "=0"){
					var ok = confirm("Applicants can only be moved to folders");
					window.location.reload();
				}
				//action=changeOrder&elementId=9fa148dc-81f6-4704-bec8-2cc57976f7d2&destOwnerEl=1_In Screening&position=3&oldOwnerEl=1_Cleared-Interview Round-1
			}
