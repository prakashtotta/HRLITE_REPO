...
$.ajax({
	type: 'GET',
  url: "jsp/home/targetSlipingDateAdmin.jsp"+"?ddd="+(new Date).getTime(),
  success: function(data){
...
$.ajax({
	type: 'GET',
  url: "jsp/home/filledActiveRequistionAdmin.jsp"+"?ddd="+(new Date).getTime(),
  success: function(data){
...
$.ajax({
	type: 'GET',
  url: "jsp/home/onBoradJoinedAdmin.jsp"+"?ddd="+(new Date).getTime(),
  success: function(data){
...
$.ajax({
	type: 'GET',
  url: "jsp/home/reqOpenStatus.jsp"+"?type=admin&ddd="+(new Date).getTime(),
  success: function(data){
...
$.ajax({
	type: 'GET',
  url: "jsp/home/dashboard.jsp"+"?ddd="+(new Date).getTime(),
  success: function(data){
  $('#reportdata').html(data);
	//document.getElementById("mytaskloading").style.visibility = "hidden";	
  }
...
<%
boolean isReportViewPermission = PermissionChecker.isPermissionApplied(Common.REPORTS_VIEW, user);
...
<DIV id="outer-center" class="hidden">
	...
	<UL id="tabbuttons" class="hidden">
		...
		<% if(isReportViewPermission){%>
		<%if(!StringUtils.isNullOrEmpty(user.getPackagetaken()) && Constant.isPackageContainFunction(user.getPackagetaken(),"reports")){%>
		<LI class="tab3"><A href="#tab3">Reports</A></LI>
		<%}%>
		<%}%>
		<%
			String domainurl = "http://"+userregdata.getSubdomain()+"."+Constant.getValue("domain.name");
	    %>
	</UL>

<div style="position: absolute;right: 20px;">
<% if(BOFactory.getUserBO().isPackageExpiredSoon(user.getSuper_user_key())){%>
<span style="color: brown;">Your free trial expires soon. Upgrade today to keep your account open </span><a  style="color: blue;text-decoration: underline;" href="http://www.hires360.com/pricing.html" target="new"> View Upgrade Options >> </a>
&nbsp;&nbsp;
<%}%>
...
<a  style="color: blue;text-decoration: underline;" href="#" onClick="getwidgetcode()">Widget code</a>
&nbsp;&nbsp;
<a  style="color: blue;text-decoration: underline;" href="#" onClick="openURLNew('<%=domainurl%>')"><%=userregdata.getSubdomain()+"."+Constant.getValue("domain.name")%></a>
<a  href="jobboard.do?method=editjobboard" title="edit my job board"><img src="jsp/images/ico_edit.png" width="20" height="20" border="0"/></a>
<%}%>
</div>
	<DIV id="tabpanels">
...
int mytasksize = TaskBO.getCountOfPendingTasksBysearchTypeandAssignedby(user.getUserId(),null,"0");
	%>
				<UL>
					<LI class="tab1"><A href="#simpleTab1"><%=Constant.getResourceStringValue("home.dashboard",user.getLocale())%></A></LI>
					<% if(mytasksize>0){%>
					<LI class="tab2"><A href="#simpleTab2"><img src="jsp/images/icon_yellow_star_highlight.png" border="0" width="20" height="20"/><%=Constant.getResourceStringValue("home.mytasks",user.getLocale())%></A></LI>
					<%}else{%>
					<LI class="tab2"><A href="#simpleTab2"><%=Constant.getResourceStringValue("home.mytasks",user.getLocale())%></A></LI>
					<%}%>
				...
				<!--dashboard end-->
					<DIV id="simpleTab2" style="height:680px">
...
%>
	...
	<DIV class="ui-widget-header ui-corner-top button1"><%=Constant.getResourceStringValue("home.onBoards_applicants",user.getLocale())%> ( <%=Constant.getValue("dashboard.noofdays.offset.config")%> <%=Constant.getResourceStringValue("home.days",user.getLocale())%>)</DIV>
	<DIV class="ui-widget-content">
	...
	<DIV class="ui-widget-header ui-corner-top button1"><%=Constant.getResourceStringValue("home.important_links",user.getLocale())%></DIV>
	<DIV class="ui-widget-content">
	...
	<DIV class="ui-layout-east">
		   ...
		   <% if(isReqSlipingTargetVisible.equals("true")){%>
			<DIV class="ui-layout-north">
				...
				<DIV class="ui-widget-header ui-corner-top button1"><%=Constant.getResourceStringValue("home.req_slipping_target",user.getLocale())%></DIV>
				<DIV class="ui-widget-content">
		   ...
		   <% if(isOfferDeclinedVisible.equals("true")){%> 
			...
			<DIV class="ui-layout-center">
				<DIV class="ui-widget-header ui-corner-top button1"><%=Constant.getResourceStringValue("home.req_open_state",user.getLocale())%></DIV>
				<DIV class="ui-widget-content">
		...
		<% 
		   ...
		   <% if(isRecentIntVisible.equals("true")){%> 
			<DIV class="ui-layout-south">
				<DIV class="ui-widget-header ui-corner-top button1"><%=Constant.getResourceStringValue("home.recent_interview",user.getLocale())%>
				<% if(user.getMenu() != null && user.getMenu().isCalendarAccess()){%>
				&nbsp;&nbsp;<a  style="color: blue;text-decoration: underline;" href="calendar.do?method=calendarView"><%=Constant.getResourceStringValue("home.calendar",user.getLocale())%></a>
				<%}%>
				</DIV>
	...
	var formatOwner = function(elCell, oRecord, oColumn, sData) {
	        ...
	        if(oRecord.getData("isGroup")=='Y'){
			 ...
			 elCell.innerHTML = "<a  href='#' onClick=userDetailsOrGroup(" +"'"+ oRecord.getData("currentOwnerId")+ "'"+","+"'"+ oRecord.getData("isGroup")+ "'"+ ");return false;"+ ">" + ndata1 + "</a>";
			  ...
			  elCell.innerHTML = "<a class='personPopupTrigger' rel='"+oRecord.getData("currentOwnerId")+","+oRecord.getData("isGroup")+"' href='#' onClick=userDetailsOrGroup(" +"'"+ oRecord.getData("currentOwnerId")+ "'"+","+"'"+ oRecord.getData("isGroup")+ "'"+ ");return false;"+ ">" + ndata1 + "</a>";*/
...
var formatHiringMgr = function(elCell, oRecord, oColumn, sData) {
            var ndata1 = "";
				...
				elCell.innerHTML = "<a  href='#' onClick=userDetailsOrGroup(" +"'"+ oRecord.getData("hiringMgrIdValue")+ "'"+","+"'"+ "N"+ "'"+ ");return false;"+ ">" + ndata1 + "</a>";
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/home/requisitionlistpageAdmin.jsp?ddd="+(new Date).getTime()+"&");
 ...
 <%=strkeycontentassignedreq%>
    ...
    // DataTable instance
    var myDataTable = new YAHOO.widget.DataTable("DynamicDataActiveReq", myColumnDefs, myDataSource, myConfigs);
...
var formatUrlofferowner = function(elCell, oRecord, oColumn, sData) {
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/home/myofferapplicantslistpageAdmin.jsp?ddd="+(new Date).getTime()+"&");
...
var formatUrlofferowner = function(elCell, oRecord, oColumn, sData) {
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/home/targetjoiningapplicantslistpageAdmin.jsp?interviewstate=<%=Common.OFFER_ACCEPTED%>&ddd="+(new Date).getTime()+"&");
...
function edittask(id){
	...
	GB_showCenter("<%=Constant.getResourceStringValue("hr.update.task",user1.getLocale())%>",url,400,500, messageretrefresh);
}

function getwidgetcode(){
	
	var url = "<%=request.getContextPath()%>/dashboard.do?method=getwidgetcode";
	
	GB_showCenter("<%=Constant.getResourceStringValue("hr.website.widget.code.get",user1.getLocale())%>",url,400,500, messageret);
}
