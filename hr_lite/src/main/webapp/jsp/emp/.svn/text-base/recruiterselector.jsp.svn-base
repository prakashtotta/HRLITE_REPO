...
//window.opener.document.getElementById("recruiterId").innerHTML = dyvalue;

if(boxnu == "-1"){
	opener.document[PFormName].recruiterId.value=id_value;
	opener.document[PFormName].recruiterName.value=c_value;
...
self.close();
	   ...
	   document.createUserForm.action = "user.do?method=searchrecruiters&boxnumber=<%=boxnumber%>";
