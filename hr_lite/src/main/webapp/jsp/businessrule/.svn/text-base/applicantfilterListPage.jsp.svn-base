...
<%
System.out.println("*** applicantfilterListPage.jsp ****");
...
int totalSize = 0;
 ...
 dataList = BOFactory.getBusinessRuleBO().searchApplicantFilters(user1,name,variabletype,variablename,user1.getSuper_user_key(),results,startIndex,dir_str,sort_str);
 totalSize= BOFactory.getBusinessRuleBO().countsearchApplicantFilters(name,variabletype,variablename,user1.getSuper_user_key());

List newFilterList = new ArrayList();
		for(int i=0;i<dataList.size();i++){
			BusinessCriteria fc = (BusinessCriteria)dataList.get(i);
			...
			}else if(!StringUtils.isNullOrEmpty(fc.getVariableName())){
...
{"businessCriteraId","name","variableName","variableType","variableOrigin","variableCriteria","filterValue1","filterValue2","filterValueDate1","filterValueDate2","status","valueName"};
...
String data =  "{"+ "\n"+ 
               "\""+ "recordsReturned"+"\""+":"+newFilterList.size()+ ","+"\n"+ 
                "\""+  "totalRecords"+"\""+":"+totalSize+","+"\n"+ 
				 ...
				 "\""+ "dir"+"\""+":"+"\""+dir+"\""+","+"\n"+ 
				  "\""+ "pageSize"+"\""+":"+results+","+"\n"+ 
				  "\""+"records"+"\""+":"+StringUtils.createJasonDataTable(newFilterList,fields,user1)+
				 "}";
