...
BOFactory.getTreeBO().setTreeviewForRecruiter(request,user1.getUserId());
...
function getrequisitionbyorgrecruiterid(orgid,recruiterid){
		url="jobreq.do?method=recruiterjobreqtreelistajax&orgId="+orgid+"&recruiterid="+recruiterid+"&deptId=0";
	location.replace(url)
}

function getrequisitionbyorgdeptrecruiterid(orgid,deptid,recruiterid){
	url="jobreq.do?method=recruiterjobreqtreelistajax&orgId="+orgid+"&recruiterid="+recruiterid+"&deptId="+deptid;
	location.replace(url)
}
...
function getallapplicantswithstatus(reqid,st){
	url="jobreq.do?method=recruiterapplicantlist&state="+st+"&requistionId="+reqid;
	location.replace(url)
}

function getapplicantdata(appid,uuid){
	url ="applicant.do?method=recruiterApplicantDetailstree&applicantId="+appid+"&secureid="+uuid;
	location.replace(url)
}
</script>
...
<div class="data">
	...
	<b><%=Constant.getResourceStringValue("aquisition.applicant.Recruiter_applicant_tree",user1.getLocale())%> </b>
<br><br>
...
%>
<br>
 <%=Constant.getResourceStringValue("Requisition.reqlist",user1.getLocale())%>
...
<% if(method != null && !method.equals("recruiterApplicantDetailstree")){%>
...
<%}%>
 ...
 <br>
<% if(method != null && method.equals("recruiterapplicantlist")){
...
YAHOO.example.DynamicData = function() {
	...
	var formatUrl = function(elCell, oRecord, oColumn, sData) {
          elCell.innerHTML = "<a href='applicant.do?method=recruiterApplicantDetailstree&applicantId=" + oRecord.getData("applicantId") +"&secureid=" + oRecord.getData("uuid") + "'"+ " >" + sData + "</a>";
        };
...
<% if(method != null && method.equals("recruiterjobreqtreelistajax")){%>
...
var formatUrl1 = function(elCell, oRecord, oColumn, sData) {
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/talent/treeRecruiterJobrequisitionlistpage.jsp?orgId=<%=orgId%>&recruiterid=<%=recruiterid%>&deptId=<%=deptId%>&ddd="+(new Date).getTime()+"&");
...
<jscontrols-ajax:treeview-dyn tree="${requestScope.dynamicTreeview}"
							  ...
							  baseUrl="${pageContext.request.contextPath}/ajaxrecruitertree.do"
