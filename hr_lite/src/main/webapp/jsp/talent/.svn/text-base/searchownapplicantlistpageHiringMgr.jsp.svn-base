...
Map m = BOFactory.getApplicantBO().searchOwnApplicantsForPaginationHiringMgr(user1,criteria,results,startIndex,dir_str,sort_str);
...
dataList = (List)m.get(Common.APPLICANT_LIST);
 ...
 totalSize = ((Integer)m.get(Common.APPLICANT_COUNT)).intValue();
	...
	//totalSize = ApplicantBO.getCountOfsearchOwnApplicantsForPaginationHiringMgr(user1,criteria);


	String[] fields = ScreenSettingUtils.getArrayOfApplicationScreenSettingsKeyListPage(Common.APP_SEARCH_SCREEN_HIRING_MGR);

  
/*String[] fields = {"applicantId","fullName","email","phone","city","heighestQualification","interviewState","appliedDate","joineddate","targerofferdate","offerreleasedate","targetjoiningdate","jobTitle","referrerName","ownername","createdBy","uuid","reqId","offerownerId"};*/
