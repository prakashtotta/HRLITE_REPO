...
span1{color:#ff0000;}
 ...
 List screenfieldList = BOFactory.getApplicantBO().getVisibleScreenFieldList(Common.APPLICANT_SCREEN_AGENCY,user1.getSuper_user_key());
   List screenfieldListWorkTab = BOFactory.getApplicantBO().getVisibleScreenFieldList(Common.APPLICANT_SCREEN_WORK_EXP_AGENCY,user1.getSuper_user_key());
  Map<String,List<String>> screenMapWorkExp = BOFactory.getBusinessFilterBO().getVisibleAndMandatoryScreenFiledsByScreenCode(Common.APPLICANT_SCREEN_WORK_EXP_AGENCY,user1.getSuper_user_key());
    List screenfieldListEduTab = BOFactory.getApplicantBO().getVisibleScreenFieldList(Common.APPLICANT_SCREEN_EDUCATION_AGENCY,user1.getSuper_user_key());
   Map<String,List<String>> screenMapEducationDetials = BOFactory.getBusinessFilterBO().getVisibleAndMandatoryScreenFiledsByScreenCode(Common.APPLICANT_SCREEN_EDUCATION_AGENCY,user1.getSuper_user_key());
...
if(!StringUtils.isNullOrEmpty(editmode) && editmode.equals("yes")){
	aform.setFilePath(filePath);
}
...
if(typeof document.applicantForm.noticeperiod != 'undefined'){
 ...
 if (isNaN(noticeperiod)){
	...
	document.getElementById('progressbartable2').style.display = 'inline'; 
	  document.applicantForm.action = "agencyops.do?method=updateapplicantData&applicantId=<%=aform.getApplicantId()%>";
    ...
    if(typeof document.applicantForm.noticeperiod != 'undefined'){
	 ...
	 if (showalert){
        ...
        return false;
		  	...
		  	document.getElementById('progressbartable1').style.display = 'inline';  
	document.getElementById('progressbartable2').style.display = 'inline'; 
	  document.applicantForm.action = "agencyops.do?method=saveapplicantData";
	  document.applicantForm.submit();
	  // window.top.hidePopWin();
...
function addPreviousOrg(){
        ...
        var city_Previousorg = document.applicantForm.city_Previousorg.value;
		var country_id_prevorg = document.applicantForm.country_id_prevorg.value;
		var state_id_prevorg = document.applicantForm.state_id_prevorg.value;
		var lastSalary = document.applicantForm.lastSalary.value;
		var bonus = document.applicantForm.bonus.value;
		var currency_id_prevorg = document.applicantForm.currency_id_prevorg.value;
		var responsibilities = document.applicantForm.responsibilities.value;
		var employercontactName = document.applicantForm.employercontactName.value;
		var employercontactPhone = document.applicantForm.employercontactPhone.value;
...
var parameters="method=savepreviousOrgdetails&applicantid=<%=aform.getApplicantId()%>&prevOrgName="+prevOrgName+"&role="+role+"&startDate="+startDate+"&endDate="+endDate+"&reasonforleave="+reasonforleave+"&reportingto="+reportingToName+"&city_Previousorg="+city_Previousorg;
...
var url="applicantoffer.do?method=saveeducationdetails&type=<%=Common.EDUCATION_COLLEGE%>&applicantid=<%=aform.getApplicantId()%>&educationName="+educationName+"&specialization="+specialization+"&institute="+institute+"&percentile="+percentile+"&passingyear="+passingyear+"&startingYear="+startingYear;
...
function deleteresume(){
	//alert("hi ..");
	 var doyou = confirm("Are you sure you would like to delete resume ..... ( OK = yes Cancel = No)");
	  if (doyou == true){
	  document.applicantForm.action = "agencyops.do?method=deleteResume&applicantId=<%=aform.getApplicantId()%>&uuid=<%=aform.getUuid()%>&resume=delete";
	  document.applicantForm.submit();
	  }
	}
...
<div align="center" class="msg">
	...
	<tr>
			<td><<font color="white">Applicant already added by some other source.Please contact Recruiter for details.</font></td>
...
<div align="center" class="msg">
	...
	<tr>
			<td><font color="white">Applicant data is updated successfully.</font></td>
			<!-- <td> <a href="#" onclick="closewindow()">close window</a></td> -->
		</tr>
...
%>	
	        <tr>
			<td><font color="white"><%=errorval%></font></td>
			<td></td>
			</tr>
...
<% if(BOFactory.getApplicantBO().isTabVisible(Common.APPLICANT_SCREEN_WORK_EXP_AGENCY,user1.getSuper_user_key())){%>
<li><a href="#" rel="country2"><%=Constant.getResourceStringValue("aquisition.applicant.WorkExpSkills",user1.getLocale())%></a></li>
<%}%>
<% if(BOFactory.getApplicantBO().isTabVisible(Common.APPLICANT_SCREEN_EDUCATION_AGENCY,user1.getSuper_user_key())){%>
<li><a href="#" rel="country3"><%=Constant.getResourceStringValue("aquisition.applicant.Education",user1.getLocale())%></a></li>
<%}%>
</ul>
<div style="border:1px solid gray; width:100%; margin-bottom: 1em; padding: 1px">
		...
		<%
      		...
      		if(aform.getJobTitle() != null){
   				//tempurl1 = "<a href='#' onClick=window.open("+"'"+"agjob.do?method=editjobreqselector&approverstatus=yes&offerapplicant=yes&jobreqId="+aform.getRequitionId()+"'"+","+ "'"+"LocationPreview"+ "'"+","+"'"+"location=0,status=0,scrollbars=1,menubar=0,resizable=1,width=800,height=700"+"'"+")>"+aform.getJobTitle()+"</a>";
   					...
   					tempurl1 = "<a href='#' onClick=window.open("+"'"+"agjob.do?method=jobdetailsforAgency&reqid="+EncryptDecrypt.encrypt(String.valueOf(aform.getRequitionId()))+"'"+","+ "'"+"LocationPreview"+ "'"+","+"'"+"location=0,status=0,scrollbars=1,menubar=0,resizable=1,width=750,height=800"+"'"+")>"+aform.getJobTitle()+"</a>";
