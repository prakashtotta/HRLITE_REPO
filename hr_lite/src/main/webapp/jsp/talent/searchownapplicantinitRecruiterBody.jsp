...
function sendbulkresumeforscreening(appIds){
	...
	GB_showFullScreen('<%=Constant.getResourceStringValue("aquisition.applicant.send.profile.for.screening",user1.getLocale())%>', url, messageret1);
...
function markdeletebulk(appIds){
	...
	GB_showCenter('<%=Constant.getResourceStringValue("aquisition.applicant.markfordeletion",user1.getLocale())%>',url,400,600, messageret1);
...
function editapplicant(aid){
	...
	GB_showFullScreen('<%=Constant.getResourceStringValue("aquisition.applicant.editapp",user1.getLocale())%>',url,messageret1);
}
function createapplicant(){

	var url = "<%=request.getContextPath()%>/applicant.do?method=createPage";
	//parent.setPopTitle1("Create applicant");
	GB_showFullScreen('<%=Constant.getResourceStringValue("aquisition.applicant.createapp",user1.getLocale())%>',url,messageret1);
}
...
function editJobReq(jb_id,juuid){
	...
	GB_showFullScreen('<%=Constant.getResourceStringValue("Requisition.createjobreq",user1.getLocale())%>', url, messageret1);
}

	 function searchapplicant(){
	var tags = $('input[name="tags"]').val();  
	
document.applicantForm.action = "applicant.do?method=searchownapplicantRecruiter&tags="+tags;
document.applicantForm.submit();
}

	 function exporttoexcel(){
	
document.applicantForm.action = "applicant.do?method=exporttoexcelRecruiter";
document.applicantForm.submit();
}

	 function configuareColumns(){
	var url = "<%=request.getContextPath()%>/configureColumns.do?method=configureColumnsRecruiter";
...
<html:form action="/applicant.do?method=searchownapplicantRecruiter">
<div class="container">
...
<td><html:text property="fullName" size="30" styleClass="textdynamic" maxlength="600"/></td>
</tr>
<tr>
<td><%=Constant.getResourceStringValue("Requisition.job.requisition.name",user1.getLocale())%></td>
...
<%
 String strcolumncontent = ScreenSettingUtils.getApplicationScreenSettings(user1,Common.APP_SEARCH_SCREEN_RECRUITER);
 String strkeycontent = ScreenSettingUtils.getApplicationScreenSettingsKeys(Common.APP_SEARCH_SCREEN_RECRUITER);
...
<td><a class="button" href="#" onClick="configuareColumns();return false;"><%=Constant.getResourceStringValue("aquisition.applicant.Configuare_Columns",user1.getLocale())%></a></a>
</td>
...
YAHOO.widget.DataTable.prototype.deleteRowsBy = function (condition) {
    ...
    // DataSource instance
    var myDataSource = new YAHOO.util.DataSource("jsp/talent/searchownapplicantlistpageRecruiter.jsp?keywords=<%=aform.getKeywords()%>&searchpagedisplay=<%=searchpagedisplay%>&prevorg=<%=aform.getPreviousOrganization()%>&fullname=<%=aform.getFullName()%>&reqid=<%=aform.getRequitionId()%>&interviewstate=<%=aform.getInterviewState()%>&email=<%=aform.getEmail()%>&fromdate=<%=fromdate%>&todate=<%=todate%>&appliedcri=<%=aform.getAppliedcri()%>&applicantNo=<%=aform.getApplicantNo()%>&tagsname=<%=aform.getTagName()%>&ddd="+(new Date).getTime()+"&");
