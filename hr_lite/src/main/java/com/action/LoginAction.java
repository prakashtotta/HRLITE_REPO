...
public class LoginAction extends CommonNoLoginAction {
	protected static final Logger logger = Logger.getLogger(LoginAction.class);

	public ActionForward backtotalent(ActionMapping mapping, ActionForm form,
		...
		RefferalEmployee refuser = (RefferalEmployee) request.getSession()
				.getAttribute("employee_refferal_data");
		if (refuser != null) {
			User user = UserDAO.getUserByUserNameTypeEmployee(refuser
					.getUserName());
			if (user != null) {
				...
				if (user.getStatus().equals("A")) {
					...
					if ((!StringUtils.isNullOrEmpty(user.getDefaultPage()))
							&& (user.getDefaultPage().equals("DASHBOARD"))) {
						dashboardurl = "dashboard.do?method=dashboardlist&ddd="
								+ new Date().getTime();
					} else {
						dashboardurl = "dashboard.do?method=dashboardlist&ddd="
								+ new Date().getTime();
				...
				errors.add("org.apache.struts.action.GLOBAL_ERROR",
						new ActionError("error.login.suspended"));
	...
	public ActionForward backtomydetails(ActionMapping mapping,
	 ...
	 * @throws IOException
	 */
	public String facebookLoginDetails(HttpServletRequest req,
	...
	public ActionForward twitterLogin(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		logger.info("Inside logon method");
		ActionForward forward = new ActionForward();
		 Twitter twitter = new TwitterFactory().getInstance();
	        twitter.setOAuthConsumer(CommonUtils.getTwitterAppId(), CommonUtils.getTwitterAppKey());
	        request.getSession().setAttribute("twitter", twitter);
	        try {
	            StringBuffer callbackURL = request.getRequestURL();
	            logger.info("callbackURLasdadada::"+callbackURL);
	            int index = callbackURL.lastIndexOf("/");
	            callbackURL.replace(index, callbackURL.length(), "").append("/login.do?method=logon&loginFrom=twitter");
	            logger.info("callbackURLasdadada::"+callbackURL);
	            RequestToken requestToken = twitter.getOAuthRequestToken(callbackURL.toString());
	            logger.info("requestToken.getAuthenticationURL()::"+requestToken.getAuthenticationURL());
	            request.getSession().setAttribute("requestToken", requestToken);
	            forward.setRedirect(true);
				forward.setPath(requestToken.getAuthenticationURL());
	 ...
	 * @throws Exception
	 */
	private String twitterCallback(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.info("--> twitterCallback");
		 Twitter twitter = (Twitter) request.getSession().getAttribute("twitter");
	        ...
	        try {
	            ...
	            logger.info("twitter.getId():"+twitter.getScreenName());
	            twitter4j.User user = twitter.showUser(twitter.getId());
	            logger.info("Name"+user.getScreenName());
	...
	public ActionForward logon(ActionMapping mapping, ActionForm form,
		...
		if ("twitter".equals(loginFrom)) {
			username = twitterCallback(request, response);
		}else 
		if ("facebook".equals(loginFrom)) {
			...
			if (username == null) {
				...
				errors.add("org.apache.struts.action.GLOBAL_ERROR",
						new ActionError("Invalid Facbook account!!!"));
				saveErrors(request, errors);
			...
			if ((StringUtils.isNullOrEmpty(loginForm.getUsername()))
					&& (StringUtils.isNullOrEmpty(loginForm.getPassword()))) {
				errors.add("org.apache.struts.action.GLOBAL_ERROR",
						new ActionError("error.emailpassword.required"));
			} else if ((StringUtils.isNullOrEmpty(loginForm.getUsername()))
					&& (!StringUtils.isNullOrEmpty(loginForm.getPassword()))) {
				errors.add("org.apache.struts.action.GLOBAL_ERROR",
						new ActionError("error.email.required.login"));
			} else if ((StringUtils.isNullOrEmpty(loginForm.getPassword()))
					&& (!StringUtils.isNullOrEmpty(loginForm.getUsername()))) {
				errors.add("org.apache.struts.action.GLOBAL_ERROR",
		...
		if (username != null) {
			...
			if ((user != null)
					&& (BOFactory.getUserBO().isPackageExpired(user
							.getSuper_user_key()))) {
			...
			if ((user != null) && (user.getStatus().equals("A"))) {
				...
				user.setMenu(menu);
				request.getSession().setAttribute("user_data", user);
				String dashboardurl = "dashboard.do?method=dashboardlist&ddd="
						+ new Date().getTime();
				if ((!StringUtils.isNullOrEmpty(user.getDefaultPage()))
						&& (user.getDefaultPage().equals("DASHBOARD"))) {
					dashboardurl = "dashboard.do?method=dashboardlist&ddd="
		...
		User user = null;
		if ((!StringUtils.isNullOrEmpty(Constant.getValue("ldap.authonticate")))
				&& (Constant.getValue("ldap.authonticate").equals("yes"))) {
			try {
				...
				if (isSucces) {
					...
					errors.add("org.apache.struts.action.GLOBAL_ERROR",
							new ActionError(Common.LOGIN_ERROR_MESSAGE));
					saveErrors(request, errors);
					return mapping.findForward("failure");
				}
			} catch (Exception e) {
			...
			user = UserDAO.isLoginSuccessTypeEmployee(username,
					EncryptDecrypt.encrypt(pwd));
			if ((user != null)
					&& (BOFactory.getUserBO().isPackageExpired(user
							.getSuper_user_key()))) {
				return mapping.findForward("packageexpired");
		...
		if ((user != null) && (user.getStatus().equals("A"))) {
			...
			if ((remme != null) && (remme.equals("on"))) {
				Cookie userCookie = new Cookie(
						Constant.getValue("cookie.name"), user.getUserName());
				userCookie.setPath("/");
				if (Constant.getValue("cookie.age") != null) {
					userCookie.setMaxAge(new Integer(Constant
							.getValue("cookie.age")).intValue());
				}
		...
		logger.info("Logon Fail");
		errors.add("org.apache.struts.action.GLOBAL_ERROR", new ActionError(
				Common.LOGIN_ERROR_MESSAGE));
		saveErrors(request, errors);
		forward = mapping.findForward("failure");
	...
	public ActionForward ssologon(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		if ((!StringUtils.isNullOrEmpty(securekey))
				&& (EncryptDecrypt.decrypt(securekey).equals(Constant
						.getValue("_SHARED_SECRET_")))) {
			issecuretrue = true;
		}
		if (!issecuretrue) {
		...
		User user = UserDAO.getUserByUserNameTypeEmployee(EncryptDecrypt
				.decrypt(username));
		if ((user != null)
				&& (BOFactory.getUserBO().isPackageExpired(user
						.getSuper_user_key()))) {
	...
	public ActionForward logout(ActionMapping mapping, ActionForm form,
		...
		logger.info("username from cookie" + username);
		if (username != null) {
			Cookie userCookie = new Cookie(Constant.getValue("cookie.name"),
					username);
			userCookie.setPath("/");
	...
	public ActionForward forgotpasswordsubmit(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if (user == null) {
			...
			EmailTask emailtasknew = new EmailTask(from, tonew, ccnew, null,
					replyto, "dummysubject", null, "dummybody", null, 0, null);
