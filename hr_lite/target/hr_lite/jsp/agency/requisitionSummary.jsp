...
<bean:define id="jobreqform" name="jobRequisitionForm"
	type="com.form.JobRequisitionForm" />
<head>
<style>
.today {
	COLOR: black;
...
<html:form action="/jobreq.do?method=editjobreq"
	...
	<table width="105%">
		<tr bgcolor="#edf5ff">
			...
			<%=Constant.getResourceStringValue("Requisition.tempname",user1.getLocale())%>:&nbsp;<b><%=jobreqform.getTemplateName()%></b>&nbsp;
 ...
 onMouseout="hideddrivetip()"><img
					src="jsp/images/iconQuestionMark.gif" border="0" height="17"
					width="17" /></a><%=Constant.getResourceStringValue("Requisition.jobreqname",user1.getLocale())%></td>
			...
			<tr>
				...
				<td><%=jobreqform.getJobPosition()%></td>
			</tr>
			<tr>
				...
				String reqcruiterurl = null;
					...
					if (jobreqform.getRecruiterId() != 0) {
						...
						if(!StringUtils.isNullOrEmpty(jobreqform.getIsgrouprecruiter()) && jobreqform.getIsgrouprecruiter().equals("Y")){
							...
							reqcruiterurl = "<img src='jsp/images/User-Group-icon.png.gif'><a href='#'  onClick=window.open(" + "'" + "usergroup.do?method=editusergroup&readPreview=2&usergroupid=" + jobreqform.getRecruiterId() + "'" + "," + "'" + "_blank" + "'" + "," + "'" + "location=0,status=0,scrollbars=1,menubar=0,resizable=1,width=500,height=500" + "'" + ")>" + jobreqform.getRecruiterName() + "</a>";
						}else{
			...
			<table border="0" width="100%">
				...
				<tr bgcolor=<%=bgcolor%>>
					<td><a
						...
						target="new"><%=applicant.getFullName()%></a></td>
