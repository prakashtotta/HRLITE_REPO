...
function requistionDetails(id){
	var url = "<%=request.getContextPath()%>/jobreq.do?method=editjobreqselector&approverstatus=yes&offerapplicant=yes&jobreqId="+id;
	//window.open(url, "SearchBudgetCode","location=0,status=0,scrollbars=1,menubar=0,resizable=1,width=800,height=600");
...
<style type="text/css">
   ...
   #chartrequisitionssummaryRecruiter
	{
		width: 600px;
		height: 150px;
	}

	#chartrequisitionssummaryopenbyrecruiter
	{
		width: 1200px;
		height: 300px;
	}
...
String url = Constant.getValue("internal.url") + "jsp/yahoo/build/charts/assets/charts.swf";
		 Map m = BOFactory.getDashBoardBO().createJSONGraphForRecruiterRequistionSummary(user.getUserId(),user);
String details = (String)m.get("detail");
String fields = (String)m.get("fields");
String seriesdef = (String)m.get("seriesdef");

		 Map m2 = BOFactory.getDashBoardBO().createJSONGraphForRecruiterOpenRequistionSummary(user.getUserId(),user);
...
<td>
<br>
 Assigned to me Requisitions  <br>
...
<td>
<span class="chart_title1">Requisitions summary year wise</span>
<div id="chartrequisitionssummaryRecruiter"></div>	
</td>
</tr>
</table>
...
<tr>
<td>
<span class="chart_title1">Open Published Requisitions summary </span>
<div id="chartrequisitionssummaryopenbyrecruiter"></div>	
</td>
...
//--- chart
	...
	var clickPieChart = function (ev) {
	 ...
	 alert("Have event object " + ev.item.recruiterrequistionummary  + ev.seriesIndex ); 
	...
	var mychart = new YAHOO.widget.ColumnChart( "chartrequisitionssummaryRecruiter", myDataSource1,
	{
		series: seriesDef1,
		xField: "recruiterrequistionummary",
		yAxis: currencyAxis1,
		xAxis: categoryAxis,
		style:
		{
...
//--- chart
	...
	var clickPieChart = function (ev) {
	 ...
	 alert("Have event object " + ev.item.recruiterrequistionummaryopen  + ev.seriesIndex ); 
	 alert("Have event object " ); 
	}
	...
	var mychart = new YAHOO.widget.ColumnChart( "chartrequisitionssummaryopenbyrecruiter", myDataSource1,
	{
		series: seriesDef1,
		xField: "recruiterrequistionummaryopen",
		yAxis: currencyAxis1,
		xAxis: categoryAxis,
		style:
		{
...
var formatUrl = function(elCell, oRecord, oColumn, sData) {
		var ndata = sData + '&nbsp;<img src="jsp/images/open_button.gif" border="0" alt="edit job requisition" title="<%=Constant.getResourceStringValue("Requisition.editwindow",user.getLocale())%>" height="20"  width="19" align="middle"/>';
		var saveasdata =  '<img src="jsp/images/saveas.gif" border="0" alt="save as job requisition" title="<%=Constant.getResourceStringValue("Requisition.saveasjobreq",user.getLocale())%>" height="20"  width="19" align="middle"/>';
        var editreqd = "<a href='#' onClick=requistionDetails('" + oRecord.getData("jobreqId") + "')"+ ">" + sData + "</a>";
		...
		{key:"templateId", hidden:true},		
            {key:"jobreqName", label:"<%=Constant.getResourceStringValue("Requisition.jobreqname",user.getLocale())%>", sortable:true, resizeable:true,formatter:formatUrl},
             {key:"projectcodeValue", label:"<%=Constant.getResourceStringValue("admin.ProjectCode.pcode",user.getLocale())%>", sortable:false, resizeable:true},
				{key:"state", label:"<%=Constant.getResourceStringValue("Requisition.state",user.getLocale())%>", sortable:true, resizeable:true},
...
var formatUrljobreq = function(elCell, oRecord, oColumn, sData) {
			var ndata1 = sData + '&nbsp;<img src="jsp/images/open_button.gif" border="0" alt="edit job requisition" title="<%=Constant.getResourceStringValue("Requisition.editwindow",user.getLocale())%>" height="20"  width="19"  align="middle"/>';
         elCell.innerHTML = "<a href='#' onClick=requistionDetails('" + oRecord.getData("reqId") + "')"+ ">" + sData + "</a>";
...
var formatUrlofferowner = function(elCell, oRecord, oColumn, sData) {
		 ...
		 elCell.innerHTML = "<a href='#' onClick=userDetailsOrGroup(" +"'"+ oRecord.getData("offerownerId")+ "'"+","+"'"+ oRecord.getData("isofferownerGroup")+ "'"+ ")"+ ">" + ndata1 + "</a>";
