...
<title> <%=Constant.getResourceStringValue("hr.user.Vendor_Selector",user1.getLocale())%> </title>
...
if(boxnu == "systemrule"){
    ...
    var getuseridlist = window.opener.document['systemRuleForm'].users.length;
	var check1="false";	
	var idarray=new Array();
    var idarraynew1=new Array();
	if(getuseridlist > 0){
		for(i=0;i<getuseridlist;i++){
	   idarray[i]=window.opener.document['systemRuleForm'].users.options[i].value+",";  
	  	}
...
self.close();
	...
	alert("<%=Constant.getResourceStringValue("hr.vendor.vendorselector.deselectedmessage",user1.getLocale())%>");
}
...
self.close();
	...
	function savedata(fields){
		...
		window.location.href = "user.do?method=vendorlistsearch&boxnumber=<%=boxnumber%>" +"&close=yes";
		if(checkval=="true"){
	   ...
	   document.createUserForm.action = "user.do?method=vendorlistsearch&boxnumber=<%=boxnumber%>";
	   document.createUserForm.submit();
...
<tr>
	...
	<tr>
		<td colspan="4"><input type="button" name="search" value="<%=Constant.getResourceStringValue("hr.button.search",user1.getLocale())%>" onClick="searchcri()" class="button">
		<input type="button" value="<%=Constant.getResourceStringValue("hr.button.reset",user1.getLocale())%>" name="reset" onclick="resetdata()" class="button"/>
