...
var gDefaultPage = "jsp/js/loading.html";
		...
		'<div id="popupInner">' +
			'<div id="popupTitleBar">' +
				...
				'<div id="popupControls">' +
					'<img src="jsp/images/close.gif" onclick="javascript:close_modal();" id="popCloseBox" />' +
				'</div>' +
			'</div>' +
...
function showPopWin(url, width, height, returnFunc, showCloseBox) {
	...
	if (url.indexOf("?") == -1) 
    url += "?cacheHack=" + Math.floor(Math.random()*99999); 
  else 
    url += "&cacheHack=" + Math.floor(Math.random()*99999); 
...
function closedialodb(){
window.top.hidePopWin();
}
function canceldb(){
document.getElementById('divConfMessage').style.display = 'none';
}
function ShowMessage()
{
  SetText('Yes', 'No');
 
  DisplayConfirmMessage('are you sure', 250, 90)
